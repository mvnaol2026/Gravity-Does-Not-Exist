## Chapter 27. THE MICHELSON EXPERIMENT AND WHY THE WRONG ETHER WAS SEARCHED FOR

At the end of the 19th century, physicists were convinced that light is a wave. And if it is a wave, there must be a medium in which it propagates. Sound propagates in air, waves propagate in water. Therefore, light must propagate in some universal medium called ether. The Earth moves around the Sun at about 30 km/s. Therefore, it moves through this ether. And if it moves, an “ether wind” should arise — just as a person running on a windless day feels an oncoming flow of air.
The logic was simple. If light propagates in ether, then its speed relative to the Earth should depend on direction. Against the “wind” — slightly slower. With the “wind” — slightly faster. The difference would be small, but measurable.
In 1887, the American physicists Albert Michelson and Edward Morley performed an extremely precise experiment. The device — an interferometer — split a beam of light into two perpendicular beams, sent them in different directions, and then recombined them. If the speed of light differed even slightly depending on direction, this would appear in the interference pattern — a characteristic set of alternating bright and dark fringes. The instrument was sensitive enough to detect even a tiny fraction of the expected difference.
The result shocked the scientific world. No difference was found. In any direction. At any orientation of the instrument. Light propagated at the same speed regardless of direction and regardless of the Earth’s motion.
The conclusion physics drew was: ether does not exist. There is no medium. Light propagates in a vacuum. This conclusion became a foundation of the special theory of relativity by Albert Einstein.
But here lies the fundamental mistake — not in the experiment, but in its interpretation. More precisely, in the assumption of what the ether should be.
Physicists of the 19th century imagined ether as something like a gas or a fluid — a rarefied medium through which the Earth moves like a ship through water, creating waves and encountering resistance. This is the ether Michelson was looking for. And this is the ether he did not find.
But aol space is fundamentally different.
It is not a rarefied gas through which the Earth “flows.” It is the densest possible packing of aols — a medium that fills all space without any gaps. The Earth does not move through it like a ship through water. The Earth itself is made of aols and is immersed in a medium of the same aols. There is no boundary between the Earth’s surface atoms and the surrounding aols in the sense that there is between a ship and water.
That is why there is no “ether wind” and cannot be. Wind appears when a body moves through a medium with which it is not structurally connected. And most importantly — there are no discontinuities in the dense packing of aols; their number in any direction of light propagation is constant, determined by the rule: “aol to aol,” tightly packed.
Now about the speed of light. In the aol model, light is not a flying particle and not a wave in a moving medium. It is the transmission of vibrations from aol to aol. The speed of this transmission is determined solely by the properties of the aols themselves: their stiffness and packing density. It does not depend on the motion of the source or on the direction of propagation.
When an atom of the source strikes a neighboring aol, a vibration arises. It is then transmitted along a chain of aols at a constant speed determined by the properties of the medium. The velocity of the source is neither added nor subtracted — because the source does not “throw” any object. It only initiates a process in the medium. And the medium transmits that process at its own intrinsic speed.
The time it takes for the vibration to reach the observer depends on only one thing — the number of aols between the source and the eye. The more aols in the path, the longer the transmission. Direction plays no role, because the medium is the same in all directions. No wind. No drift. No difference between “against the flow” and “with the flow” — because there is no flow.
That is why Michelson detected no difference in the speed of light. Not because the medium does not exist, but because the medium he was looking for behaves completely differently from what he assumed. He was searching for an ether — a moving medium with flows and winds. 
But aol space is something fundamentally different: an absolutely homogeneous, isotropic medium, fundamentally at rest, in which the speed of transmission of vibrations is the same in all directions by its very nature.
The Michelson experiment did not refute the existence of a medium. It refuted a specific and incorrect model of that medium.
The Michelson experiment is one of those cases where the aol model offers a more natural explanation than standard physics. Standard physics was forced to introduce the postulate of the constancy of the speed of light and build the entire theory of relativity upon it. In the aol model, the constancy of speed follows naturally from the nature of the medium itself — without additional postulates. The aol model does not generate paradoxes or phenomena. It does not bend space, nor does it slow down time.
The Michelson experiment revealed not the absence of a medium, but the limits of how it was imagined. Physics abandoned ether not because it does not exist, but because it did not fit the familiar mechanics of fluids and gases. As a result, the idea of emptiness was accepted — and with it came postulates, paradoxes, and the need to explain phenomena without a carrier. The aol model does the opposite: it restores the medium, not as a hypothesis, but as a necessary condition for any process. Light no longer requires “duality,” abstract fields, or postulates. It becomes what it should be in a mechanical universe — the transmission of motion through contact and pressure. And then it becomes clear: the problem was not in the Michelson experiment. The problem was that the wrong reality was being searched for.






## Chapter 28. HOW RELATIVITY THEORY EMERGED — AND WHAT WOULD HAPPEN IF A MEDIUM EXISTED

At the end of the 19th century, physics faced a serious contradiction. On one side was the principle of relativity formulated by Galileo Galilei, known for two centuries: the laws of nature are the same for all observers moving uniformly and in straight lines. In a train moving at constant speed, a cup of tea behaves exactly as it does on the platform. No experiment inside the carriage can determine whether the train is moving or standing still.
On the other side were the equations of James Clerk Maxwell describing electromagnetism. They implied that the speed of light is about 300,000 km/s. But relative to what? If relative to the observer, then an observer moving toward the light beam should measure a higher speed, and one moving in the same direction should measure a lower one. This seemed obvious — this is how all velocities behave in classical mechanics.
However, the Michelson–Morley experiment showed something entirely different. No matter how precisely measurements were made, the speed of light remained the same in all directions. The Earth moves around the Sun at 30 km/s — yet the speed of light does not change at all.
Imagine the situation physicists faced at the time. If you are driving a car and turn on the headlights, then according to Isaac Newton, the light from your headlights should travel faster than light from a roadside lamp — because the car’s speed adds to the speed of light. But experiments showed: no. The speed of light is the same for everyone — whether standing still or moving toward the beam.
Albert Einstein solved this puzzle in a radical way. He accepted the constancy of the speed of light as an axiom — a fundamental law of nature requiring no explanation. Then he asked: if the speed of light is constant, what must change?
The answer was unexpected — time and space must change.
Time can slow down.
Objects can contract.
Simultaneity depends on the observer.
Thus, in 1905, the special theory of relativity was born.
Mathematically, it is flawless.
Technologically, it works — without relativistic corrections, GPS navigation would accumulate errors of several kilometers within a day. But mechanically, it left a vast gap.
Why does time slow down?
What exactly happens to a moving clock?
What physically contracts a moving rocket?
These questions are answered mathematically — but not mechanically.
Einstein did the only thing he could after the ether was abandoned. By removing the medium, he lost the mechanism and had to replace it with geometry. Space-time bends, stretches, and contracts.
This works mathematically, but is physically difficult to imagine.

Now let us look at the same situation from the perspective of the aol model — and we will see that no revolution in the understanding of space and time is required.
In the aol model, the speed of light is the speed of transmission of vibrations through the aol medium. It is determined by one thing: the stiffness and packing density of aols. This is a property of the medium itself — constant, identical everywhere and in every direction. That is why the speed of light is constant — not because it is some mysterious law of nature, but because the medium is homogeneous.
Why is the velocity of the source not added to the speed of light? Because the source does not “throw” any object and does not emit photons. It simply strikes neighboring aols, creating a vibration in the medium. This vibration then propagates at a speed determined by the properties of the medium — just as a удар on a rail produces sound whose speed depends on the properties of the metal, not on the strength of the удар.
No matter how fast the source moves, the speed of the wave in the medium remains unchanged. Light arises at the moment of the first impact of an atom on an aol of space, and the velocity of the atom does not affect the process of light formation. One can strike more strongly or more weakly, but this does not affect how the impacts are transmitted from aol to aol.
Time dilation in this picture is not a property of space-time, but a consequence of real mechanical pressure. A rapidly moving body creates strong aol wedges and encounters increased resistance from the medium. This pressure slows down the internal oscillations of the body’s atomic structures — and thus slows down all processes within it. A clock runs slower not because time itself flows differently, but because the mechanism of the clock operates more slowly under the pressure of the medium.
The effect of the medium can be visualized using an hourglass: if the chamber is filled with air or water, the flow rate changes. Change the properties of the medium — and the clock slows down.
The contraction of moving bodies is not a geometric effect of space-time, but a real mechanical compression. The pressure of the aol medium in front literally compresses the structure of the body in the direction of motion. This is exactly how the effect was explained by Hendrik Lorentz and George FitzGerald even before Einstein — through ether pressure. Albert Einstein removed the ether and reformulated contraction geometrically. The aol model restores the mechanical mechanism of Lorentz — but now with a concrete description of the medium.

Why a Maximum Speed Exists
Can a body accelerate indefinitely? At first glance — yes. If a force is continuously applied, velocity should increase without limit. This is how Newtonian mechanics works in empty space without resistance.
But in the aol model, empty space does not exist. There is a dense medium — and it is this medium that defines the limit.
Recall the mechanism of motion. A body moves because the aol medium drives wedges behind it. Each wedge is a real physical process: aols receive an impulse, undergo micro-deformation, and transmit the effect further. This process is not instantaneous. Each aol requires time to receive the impact, deform, and pass the motion to the next one. This micro-deformation — the finite stiffness of aols — is precisely the reason for the finite speed of light. An absolutely rigid aol would transmit effects instantly. 
A real aol does so with a delay. This delay determines the speed of propagation of any interaction in the medium.
At low speeds, the medium has time to reorganize. Wedges form steadily, the support from behind is stable, and acceleration continues.
As speed increases, the situation changes. The body begins to move faster than the medium can reorganize behind it. Wedges form less effectively — aols do not have time to fill the space behind the body before it moves forward. The support weakens. Acceleration decreases.
At higher speeds, a limit is reached. The aol medium can no longer form new wedges at the required rate — the reconfiguration speed of the medium is exhausted. The support from behind stops increasing. Acceleration ceases.
This limit is the speed of light.
It is not an abstract prohibition of nature. Not a geometric property of space-time. It is a concrete physical quantity — the maximum speed at which the aol medium can transmit and reorganize motion. The speed of light is the speed of the medium itself. A body cannot move faster than the medium can sustain it.
That is why the speed of light is both the maximum speed of motion for bodies and the speed of propagation of any interaction in aol space. This is not a coincidence — it is the same quantity determined by the same property of the medium: the finite stiffness of aols and the finite rate of their micro-deformation.
Thus, all the seemingly paradoxical consequences of special relativity receive simple mechanical explanations in the aol model. The constancy of the speed of light follows from the homogeneity of the medium. 
Time dilation follows from the slowing of internal oscillations under pressure. Length contraction follows from real mechanical compression by the medium. The speed limit follows from the limits of the medium itself.
Relativity arose as a forced answer to the question: how to explain the constancy of the speed of light without a medium? 
The aol model asks a different question: why remove the medium at all? 
If the medium exists — and exists as described by the aol model — all these effects follow naturally from mechanics, without paradoxes and without the geometry of four-dimensional space-time.
Albert Einstein was forced to seek a mathematical solution where a physical solution was hidden by the absence of a correct model of the medium. Not because he was mistaken, but because the ether, as imagined by 19th-century physicists, truly does not exist. 
What exists is something different — aol space. 
And with it, the picture of the world becomes simpler, not more complex.






## Chapter 29. A BRIEF SUMMARY OF LIGHT, MEDIUM, AND RELATIVITY

Modern physics encountered a fact — the speed of light is constant.
But by abandoning the medium, it lost the mechanism.
And so it was not the medium that was made invariant, but space and time.
Thus appeared: time dilation, length contraction, and relativity of simultaneity.
This solution works mathematically.
But it does not answer the main question: what physically happens?
The aol model takes the opposite path.
It does not change space and time. It neither curves nor slows them down.
In the Aol model, time is not an entity, but a characteristic of the duration of processes introduced by humans for the convenience of calculations.
The Aol model returns the medium.
And with the medium, the mechanism returns.
Light is not particles and not abstract waves. It is the transmission of motion.
The speed of light is not a postulate. It is a property of the medium.
The speed limit is not a prohibition. It is the limit of the medium’s reconfiguration.
Time dilation is not a change of time. It is a slowing of processes.
Contraction is not geometry. This is a result of pressure.
All the effects that appear as paradoxes in relativity
become consequences of a single process in the aol model — the transmission of motion through contact and pressure.
Without postulates.
Without abstractions.
Without the geometry of four-dimensional space-time.
Only the medium and its behavior.
And then it becomes clear that relativity is not a final explanation,
but a mathematical description of phenomena that arose from the absence of a proper model of the medium.
The aol model takes the next step.
It restores what no mechanics can exist without —
the carrier.
And with this restoration,
the picture of the world becomes coherent.






## Chapter 30. ELECTRIC CURRENT

When a person turns on a light or starts a motor, they rarely think about what is actually happening inside the wires. Science gives a simple answer: electrons are moving. This has become the standard explanation — simple, convenient, and at first glance complete. Few people question it. Electrons — so be it.
But if we examine this picture closely — mechanically, from the inside — it falls apart.

The Insignificance of the Electron
Let us begin with a simple estimate. If we were to gather all the electrons on Earth — not only those involved in electrical processes, but all of them, including those in your own atoms — into a single cluster, it would be invisible under any microscope. To make electrons occupy the volume of half a finger segment, one would need the electrons of a billion Earth-like planets. A billion.
In real electrical processes, only free electrons participate — roughly one per atom of the conductor. This is incomparably less than all the electrons of the planet. Even if gathered together, this matter would remain unobservable.
And it moves at a speed of about a tenth of a millimeter to a few millimeters per second. Touch your fingers together and slowly separate them by one centimeter over ten seconds. That is the scale of motion of the matter to which we attribute all electrical power — billions of light bulbs, millions of motors, electric trains, electromagnets moving massive loads.
Think about this calmly. On one side — a barely creeping, invisible cluster. On the other — the entire electrical power of a planet. Something clearly does not add up.

A Walk Inside the Conductor
Let us step inside a copper wire and observe what is happening.
Scale atoms up so that the nucleus becomes the size of a grain of sand — about a millimeter across. Then the entire atom would be about one hundred meters in diameter. A hundred-meter sphere of almost empty space. Somewhere about fifty meters from the nucleus, electrons “move” — and even at this enormous scale, they remain invisible.
The mass of a copper nucleus exceeds the mass of an electron by about 117,000 times. This is not just a difference — it is an extreme disparity. Imagine a small child next to a mass of dozens of welded tanks. That is the ratio between an electron and an atom.
Now switch on the circuit. According to standard physics, the chaotic electrons — previously moving at speeds up to a hundred kilometers per second — suddenly calm down and begin to drift in one direction at about a tenth of a millimeter per second. A difference of one hundred million times. And these nearly motionless electrons now begin to heat the conductor. A paradox: fast electrons did not heat, but slow ones do.
But the main question is not even this. How can an electron set an atom into motion? A child moving at a tenth of a millimeter per second runs into a pile of tanks. How could she move them? She cannot. And the child hits tanks — but the electron meets emptiness. The atom is almost entirely empty; there is practically nothing to push against. The electron is too small, too light, too slow. Heating a filament to thousands of degrees is beyond such an insignificant object.
Science avoids this paradox with the concept of the “electric field.” The field supposedly propagates along the conductor and organizes the motion of electrons. But what is this field? What is it made of? What drives it? Why does it propagate at the speed of light? How does it navigate a twisted wire?
There are no answers.

The Tolman–Stewart Experiment
In 1916, American physicist Richard Tolman and Scottish physicist Thomas Stewart conducted an experiment considered strong evidence for the electronic nature of electric current. A coil of wire was spun at high speed and then abruptly stopped — and a brief current appeared in the circuit.
The conclusion: the inertia of electrons carried them forward after the coil stopped.
But consider this. Before the experiment, free electrons were already moving between atoms at speeds of up to a hundred kilometers per second — in all directions: forward, backward, sideways, up, and down. The rotation of the coil adds only a tiny additional component to this chaotic motion. When the coil is suddenly stopped, this tiny component disappears. An electron moving at a hundred kilometers per second in a random direction would not even “notice” this loss. No ordered current should arise from such a situation.
In the aol model, however, the experiment is explained clearly. Imagine a hose filled with water, wound onto a coil. The water spins together with the hose as a single continuous “snake.” When the coil is abruptly stopped, the water continues moving by inertia, creating pressure. In the same way, the aol “snake” inside the conductor — dense, continuous, aol to aol — continues its motion when the coil stops. That is the origin of the current. Not the inertia of randomly moving electrons, but the inertia of a unified aol flow.

What Electric Current Really Is
Electric current is not the directed motion of electrons.
It is the directed motion of aols within the conductor.
Aols fill all space — including inside any material, including a metal wire. They are packed tightly, forming a continuous “snake” along the entire conductor. When pressure is applied at one end, the entire structure shifts simultaneously — like water in a pipe when pushed by a piston. There is no need to wait for something to travel from one end to the other. Pressure is transmitted instantly.
With direct current, the flow moves in one direction. With alternating current, it oscillates back and forth at the source frequency. The “snake” does not move as a whole object — what propagates is a process, a pressure wave moving through the medium.
That is why an electrical signal propagates almost instantly despite the slow motion of individual carriers. What is transmitted is not matter — but pressure. And pressure in a dense medium propagates at a speed close to the speed of light.
The working substance of electric current is not a negligible number of electrons, but the entire aol medium permeating the conductor. 
The power of electricity is explained by the properties of this medium — dense, elastic, capable of transmitting enormous mechanical effects.
It is not a child pushing a pile of tanks — it is a boar pushing an elephant.
With force. With impact.

Heating, Light, and Resistance
The aol snake is uneven — aols are spherical, with alternating protrusions and hollows between them. As the snake moves past the atoms of a conductor, it alternately presses them into the gaps using the external aol space and pushes them out with the protrusions. 
Within one millimeter of a conductor, there are millions of aols. At a current speed of one tenth of a millimeter per second, each atom receives several million impacts per second — several megahertz of additional oscillations on top of existing thermal motion. The conductor heats up. With sufficient current — it glows. With very large current — it melts. Everything is proportional and mechanically clear.
When a filament is heated to several thousand degrees, its atoms begin oscillating at frequencies above 10¹⁴ Hz. These oscillations are transmitted to the aols of space and propagate in all directions. We perceive this as light. No photons of mysterious origin. Only high-frequency mechanical vibrations of the aol medium reaching the retina.
Resistance is not a mysterious property of materials. Atomic structures in metals oscillate, and each deviation partially scatters the directed motion of the aol “snake” sideways. Part of the directed motion becomes chaotic atomic oscillation — heat. The more obstacles along the path, the higher the resistance and the greater the heating.
This also explains the difference between conductors and insulators. In metals, the geometry of atomic structures forms continuous channels for aol motion — current flows freely. In dielectrics, these channels are blocked — the “snake” cannot pass. A semiconductor is an intermediate case: channels exist, but can open or close depending on conditions.
When cooled to a critical temperature, thermal oscillations of atomic structures nearly disappear — obstacles vanish, and current flows without resistance indefinitely. This is superconductivity.

Magnetic Field and Induction
When the aol “snake” moves inside a conductor, it sets parts of atomic structures into motion. These begin to rotate, dragging surrounding aols into circular vortices around the conductor. The stronger the current, the stronger the vortices. This is the magnetic field — not an abstract force field, but a real circular motion of aols.
Two parallel conductors carrying current in the same direction — vortices between them rotate coherently and merge. Pressure between the conductors drops below the surrounding pressure — the aol medium presses them together. The conductors are pressed together. 
If currents are opposite — vortices collide, pressure increases — conductors repel. 
Ampère’s law receives a mechanical explanation without abstractions.
Electromagnetic induction follows the same mechanics. 
When current in one conductor changes, the motion of aol vortices changes as well. 
These changing vortices reach a nearby conductor, disturb its atomic structures, and in turn push the aol “snake” inside it. 
A current arises. 
No “creation of energy from nothing” — only transmission of motion through the medium.

Summary
Voltage is the difference in pressure of the aol medium at the ends of a conductor.
Current is the directed motion of the aol “snake.”
Resistance is the loss of motion through imperfect atomic structure.
Magnetic field is vortices of aols around the conductor.
Induction is the transmission of these vortices to neighboring conductors.
Everywhere the same mechanism operates: contact, pressure, motion.
Electricity ceases to be a mystery and becomes what it truly is — the mechanics of the aol medium. 
The power of electrical processes is explained not by mysterious properties of rare invisible particles, but by the properties of a dense, continuous medium permeating all matter and capable of transmitting enormous mechanical effects.






## Chapter 31. INDUCTION — THE TRANSMISSION OF MOTION

Electromagnetic induction is considered one of the key phenomena in physics. The operation of generators, transformers, and many other devices is based on it. But if we ask what exactly happens during induction, the answer usually comes down to the phrase: “a changing magnetic field creates current.” This is a description. But not an explanation.
In the aol model, induction is the transmission of motion through a changing state of the medium. Let us examine this step by step.

Direct Current — and Why It Does Not Create Induction
When direct current flows through a conductor, vortex motions of aol space form around it. These vortices are stable — they are directed the same way and do not change over time. The medium reaches an equilibrium state.
If a second conductor is nearby, its atoms come under the influence of these vortices. Their parts turn, adjust to the motion of the medium, and take on a new position. And that is where it ends. The system has reached equilibrium. No change in the effect — no further motion — no current in the second conductor.
That is why direct current does not create sustained induction. It only rearranges the neighboring structure once — and then becomes static.

Alternating Current — Where Induction Begins
Now let us consider alternating current. When the current in the first conductor changes direction, the direction of the vortices in aol space changes along with it. The medium ceases to be stable and begins to continuously rearrange itself — first in one direction, then in the other.
This is the key point.
The atoms of the second conductor can no longer take a stable position. Their parts are forced to shift continuously, following the changing vortices of the medium — first one way, then the other. This oscillating effect begins to push the aol “snake” forward and backward. Alternating current appears in the second conductor.
Motion arises not because “a field creates something.” It arises because the medium does not allow the system to come to rest — it keeps rearranging itself and thereby continuously transmits motion further.
Here we see a simple and clear pattern. A constant effect fixes a structure. A changing effect pushes. And a change of direction creates sustained oscillatory motion.

Transformer
This is the principle on which a transformer operates.
An alternating current is created in the primary winding, forming constantly changing vortices of aol space. These vortices act on the secondary winding, forcing its atoms to continuously rearrange themselves. As a result, an alternating current appears in the secondary winding — without direct contact between the windings, but through the medium.
No transfer of energy through a mysterious field is required here. There is transmission of motion. Through aol space. Through contact. Through pressure.
And this is induction.






## Chapter 32. ELECTRIC CHARGE AND MAGNETISM AS FORMS OF AOL MOTION

In classical physics, electric charge is a fundamental property of a particle that simply “exists.”
An electron has a negative charge, a proton a positive one.
Why — is not explained. It is accepted as an initial fact that requires no justification.
In the aol model, there is no separate entity called “charge.” There is only the motion of matter.
Therefore, the question is posed differently: what state of motion of matter and aol space do we call charge?

How charge arises
Let us consider a metal body exposed to a поток света → correction needed: should be "a light flux". Let's fix properly.
Let me rewrite that sentence cleanly:
Let us consider a metal body exposed to a light flux.
In the aol model, light is not photons — that is, not corpuscles and at the same time not an abstract wave — but a flow of high-frequency impacts transmitted through aol space. These impacts, reaching the surface of the metal, are transferred to atoms and, if sufficiently intense, organize the motion of aols inside the conductor — a directed flow appears, an aol snake.
Free aols, moving through matter, do not travel through empty space but through the structure of atoms. An atom is not a smooth sphere. It is a complex volumetric construction with directed elements, with the ability of its parts to rotate, and with a specific geometry of channels. Passing through such a structure, the flow of aols does not simply move forward. It inevitably interacts with the geometry of the atom.
If this geometry is asymmetric or contains rotational elements, the flow acquires not only translational motion but also a rotational component.
It begins to move in a helical manner.
Just as a stream of water passing through a spiral pipe acquires a twist, so the flow of aols, passing through atomic structure, acquires a helical rotational organization. This is not the introduction of a new entity. It is a natural consequence of motion through a structured, curved medium.
If this flow of aols is formed not simply in a straight line but with a helical structure, it carries not only translational motion but also a rotational component. Such a flow, passing through atomic structure, begins to interact with its elements asymmetrically. It exerts pressure not only along the direction of motion but also tangentially, creating a torque.
As a result, the movable parts of atoms begin to rotate. It is important to emphasize: this is not “spontaneous rotation.” It is always the result of pressure — the aol snake physically spins the structure, transferring its motion to it.
After the flow of aols leaves the body or weakens, a key question arises: why does the rotation not disappear instantly? After all, in the model there is no internal energy that could be “stored” in the atom.
The answer is given by the medium itself.
A rotating atom is not in a vacuum. It is constantly in contact with aol space. During rotation, it displaces aols tangentially, creating an asymmetric distribution of pressure around itself. Aol space, which is under constant external pressure, tends to fill the возникающие смещения → correction:
Aol space, which is under constant external pressure, tends to fill the arising displacements. As a result, a special structure of motion of the medium is formed — rotational aol wedges.
If in translational motion wedges arise behind the body and push it forward, then in rotation they form around the rotating structure and exert pressure tangentially, supporting its motion. This is a fundamentally important point: rotation does not “persist by itself”; it is continuously sustained by the pressure of the medium, organized by this very rotation.
Thus, a closed process arises.
The atom rotates — creates an asymmetry of pressure — aol space forms wedges — the wedges continue to push in the same direction — the rotation is maintained.
This is rotational aol inertia — a complete analogue of the translational inertia of a body, but for rotational motion.
If such a configuration is formed in a significant number of atoms on the surface and in the volume of a body, a stable state arises. It does not require constant external action, because it is supported by the interaction between the structure of matter and aol space.
This state is what we call electric charge.
Charge is not a particle and not a substance. It is a stable rotational state of atomic structure, supported by aol space.

Two signs of charge
Helical motion always has two possible directions. Looking along the axis of motion, the flow can rotate clockwise or counterclockwise. These two modes are fundamentally different and do not transform into each other without a restructuring of the entire motion structure.
These two modes are exactly what physics calls positive and negative charge.
One charge is right-handed helical motion.
The other is left-handed.
Charge ceases to be a mysterious internal property of a particle. It becomes a type of organization of motion of atoms and aol space.

Electric field
Rotating atoms continuously transfer pressure to the surrounding aols, but they do so with the same rotational displacement. As a result, the surrounding aol space is involved in organized motion. A region appears in which aols move with a certain helical structure. This motion spreads outward from the body, gradually weakening, but preserving the direction of rotation.
This structured motion of aol space is the electric field.
The field is not something separate from the charge. It is a continuation of the same process, but in the medium. Charge is a state of matter.
The field is a state of aol space caused by this state.

Interaction of charges
If two bodies create flows of aols with the same helical orientation around themselves, then when they approach each other, these flows begin to interfere with each other, they collide. Their structures are incompatible. When they overlap, a local increase in pressure occurs. The medium resists and pushes the bodies apart.
This is the repulsion of like charges.
If the helical directions are opposite, the flows are compatible. They do not interfere but continue each other. Between the bodies, a region of more ordered motion and lower pressure appears. Outside, the pressure remains normal, and the medium outside presses the bodies toward each other. This is what is perceived as “attraction.”
There is no attraction. There is agreement or disagreement of motion and, as a result, a redistribution of pressure that causes the bodies to be pressed together or pushed apart. This is the same mechanism that explains the interaction of magnets.

From charge to current and magnetic field
Electric current in a conductor is not just a flow of aols. It is a flow of aols with a certain helical structure of motion. The conductor not only sets the direction but also stabilizes this structure — its atoms create a repeating geometry of interaction that prevents the flow from breaking apart and directs it.
If the helical motion is directed along the conductor, circular motion of the medium inevitably arises around it — a direct consequence of the helical geometry. A flow twisted around an axis creates a rotational effect on the atoms of the conductor, and through them on the surrounding medium. This is observed as a magnetic field around a current-carrying conductor in the form of circular vortices of aols.
The magnetic field is not a separate entity and not an abstract field. It is a structured circular motion of the aol medium caused by the helical flow inside the conductor.
If the atoms of a substance are capable of organizing stable rotation of their parts, they can maintain a flow of aols without an external current. In this case, a permanent magnet arises.
Inside it, there exists a stable organized motion of aols with a rotational structure that closes through the space outside.
The north and south poles are not “charged regions,” but points of exit and entry of this organized motion of aols.

Why charge disappears
In ideal conditions of aol space, the rotational configuration could persist for a very long time. But in reality, a body is in air. Air molecules constantly collide with the surface, disrupting the fine structure of rotational wedges and creating resistance to the rotation of atoms, converting part of the motion into chaotic vibrations. In a humid environment, this process proceeds much faster — water molecules are larger, they more effectively destroy the organized motion and more quickly damp the rotation of atoms.

Unified picture
All electromagnetic phenomena turn out to be manifestations of three fundamental forms of aol motion.
Translational motion of aols is electric current.
Helical motion of aols is charge and the electric field.
Rotational motion of aols is the magnetic field.
Charge is a type of helical motion of atomic structure supported by aol space. Its “sign” depends on the direction of rotation.
The electric field is the propagation of this helical motion of aols in the medium.
Electric current is a directed flow of such motion in a conductor.
The magnetic field is circular motion of the medium caused by the helical structure of the current. A permanent magnet is a closed, stable mode of pumping aols through the body of the magnet.
Even the photoelectric effect naturally fits into this picture.
When a light vibration tears organized motion out of a metal, what leaves is not a “particle,” but a stable fragment of helical motion of aols. This stable mode is what is identified as an electron with a certain charge.
No separate “charges” as entities. No immaterial fields. No action at a distance.
Contact. Pressure. Motion.
